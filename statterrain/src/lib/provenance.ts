...
export type HospitalProvenanceClassification =
  | "cms-only"
  | "synthetic-only"
  | "mixed"
  | "none";
...
function registrySource(id: string): any | null {
  return (
    (sourceRegistry as any).sources?.find(
      (source: any) => source.id === id || source.sourceId === id,
    ) ?? null
  );
}
...
export const hospitalSourceMetadata: UiSourceMetadata = {
  sourceOrganization:
    (cmsGenerated as any).metadata?.sourceAgency ??
    "Centers for Medicare & Medicaid Services",
  datasetTitle:
    (cmsNationalManifest as any).sourceName ??
    (cmsGenerated as any).metadata?.sourceName ??
    "CMS Hospital General Information",
  datasetIdentifier:
    (cmsNationalManifest as any).sourceDatasetId ??
    (cmsGenerated as any).metadata?.sourceId ??
    "xubh-q36u",
  officialUrl:
    cmsRegistry?.sourceUrl ??
    "https://data.cms.gov/provider-data/dataset/xubh-q36u",
  releaseLabel: (cmsNationalManifest as any).sourceReleaseDate ?? null,
  reportingPeriod: null,
  estimatePeriod: null,
  retrievedAt:
    (cmsNationalManifest as any).retrievedAt ??
    (cmsGenerated as any).metadata?.retrievedAt ??
    null,
  builtAt:
    (cmsNationalManifest as any).generatedAt ??
    (cmsGenerated as any).metadata?.generatedAt ??
    null,
  coverageStatus:
    (cmsNationalManifest as any).validationStatus ??
    (cmsGenerated as any).metadata?.validationStatus ??
    null,
};

export const acsSourceMetadata: UiSourceMetadata = {
  sourceOrganization:
    (acsManifest as any).source?.agency ??
    acsRegistry?.agency ??
    "U.S. Census Bureau",
  datasetTitle:
    (acsManifest as any).source?.dataset ??
    acsRegistry?.datasetName ??
    "American Community Survey 5-Year Estimates",
  datasetIdentifier:
    (acsManifest as any).source?.sourceId ??
    acsRegistry?.id ??
    "census-acs5-county-population-2024",
  officialUrl: "https://www.census.gov/programs-surveys/acs",
  technicalUrl:
    (acsManifest as any).source?.sourceUrl ??
    acsRegistry?.sourceUrl ??
    "https://api.census.gov/data/2024/acs/acs5",
  releaseLabel: (acsManifest as any).release ?? "2024 ACS 5-year",
  reportingPeriod: null,
  estimatePeriod:
    (acsManifest as any).estimatePeriod ??
    acsRegistry?.estimatePeriod ??
    "2020-2024",
...
export const boundarySourceMetadata = {
  ...
  datasetIdentifier:
    (boundaryManifest as any).schemaVersion ?? "county-boundaries",
  officialUrl:
...
export function classifyHospitalRecords(facilities: Facility[]) {
  ...
  const cmsCount = hospitals.filter(
    (f) =>
      !f.isSynthetic &&
      (f.sourceIds.includes("cms-hospital-general-information") ||
        f.sourceName?.includes("CMS")),
  ).length;
  const syntheticCount = hospitals.filter((f) => f.isSynthetic).length;
  const classification: HospitalProvenanceClassification =
    cmsCount > 0 && syntheticCount > 0
      ? "mixed"
      : cmsCount > 0
        ? "cms-only"
        : syntheticCount > 0
