...
import { type DemographicPercentageMetric } from "./demographicPercentages";
import type { AcsMetricId, AcsMetricValue } from "./types";
...
export interface NationalBenchmarkComparison { metricKey: AcsMetricId; county: { percentage: number | null; percentageMarginOfError: number | null; status: string; universe: string; method: string; numerator: number | null; denominator: number | null; variables: string[]; }; benchmark: { geography: "United States"; percentage: number | null; percentageMarginOfError: number | null; status: string; universe: string; method: string; numerator: number | null; denominator: number | null; variables: string[]; release: string; estimatePeriod: string; }; differencePercentagePoints: number | null; comparisonStatus: NationalComparisonStatus; displayClassification: DisplayClassification; comparisonThreshold: number; }
...
export function selectNationalBenchmarkComparison(countyMetric: DemographicPercentageMetric, countyRelease: string, countyEstimatePeriod: string): NationalBenchmarkComparison {
  ...
  else if (!benchmark || benchmark.status !== "available" || benchmark.percentage == null) comparisonStatus = "benchmark-unavailable";
  ...
  else if (countyMetric.percentageMethod !== "numerator-denominator-ratio") comparisonStatus = "method-mismatch";
