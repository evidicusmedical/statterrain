...
function cmsRecordToFacility(record:any, manifest:NationalCmsManifest): Facility | null {
  ...
  return { id:`cms-${sourceFacilityId}`, name, facilityType: "hospital", lat, lng, address: asText(record.address) ?? "Not reported in this source", city: asText(record.city), state: asText(record.state), zip: asText(record.zip), phone: asText(record.phone), website: asText(record.website), sourceFacilityId, hospitalType: asText(record.hospitalType), ownershipType: asText(record.ownershipType), emergencyServicesIndicator: asText(record.emergencyServicesIndicator), criticalAccessIndicator: asText(record.criticalAccessIndicator), countyName: asText(record.countyName), stateFips: asText(record.stateFips), countyFips: asText(record.countyFips), sourceName: asText(record.sourceName ?? manifest.sourceName), sourceDatasetId: asText(record.sourceDatasetId), sourceUrl: asText(record.sourceUrl), retrievedAt: asText(record.retrievedAt ?? manifest.retrievedAt), validationStatus: asText(record.validationStatus ?? manifest.validationStatus), geocodingConfidence: asText(record.geocodingConfidence), distanceMiles: 0, criticalAccess: critical, capabilities: [], sourceIds:[sourceFacilityId, asText(record.sourceId) ?? "cms-hospital-general-information"], freshness:"current", confidence:"high", limitations: Array.isArray(record.limitations) ? record.limitations : ["CMS Hospital General Information public-data record; unsupported clinical capability fields are not inferred."], isSynthetic:false };
}
