...
import { buildSafeTelephoneHref, formatFacilityAddress, normalizeCmsPhoneDisplay, normalizeFacilityWebsite } from "@/lib/facilityIdentity";
...
export function FacilityDetailPanel({ facility }: { facility: Facility | null }) {
  ...
  const emergencyValue = facility.emergencyServicesIndicator === "Yes" || facility.emergencyServicesIndicator === "No" ? facility.emergencyServicesIndicator : "Not reported in this source";
    ...
    <div className="flex h-full w-full max-w-none flex-col overflow-y-auto border-l-0 border-t-4 border-terrain-500 bg-terrain-50/30 p-4 sm:p-5 lg:border-l-4 lg:border-t-0" data-testid="facility-detail-panel">
      <section aria-labelledby="facility-identity-heading">
        ...
        <dl className="mt-3 rounded-lg border border-slate-200 bg-white px-4">
          ...
          <DetailRow label="CMS facility ID" value={facility.sourceFacilityId ?? facility.sourceIds[0]} status={isCmsRecord ? "CMS facility_id" : syntheticValue} />
          ...
          <DetailRow label="Hospital type" value={facility.hospitalType} status="CMS hospital_type" />
          <DetailRow label="Critical access" value={facility.criticalAccess ? "Yes" : undefined} status="Mapped only from CMS hospital_type = Critical Access Hospitals" />
          <DetailRow label="Ownership" value={facility.ownershipType} status="CMS hospital_ownership" />
        </dl>
      ...
      <section className="mt-5" aria-labelledby="facility-location-heading">
        <h2 id="facility-location-heading" className="text-sm font-semibold uppercase tracking-wide text-slate-500">Location</h2>
        <dl className="mt-3 rounded-lg border border-slate-200 bg-white px-4">
          <DetailRow label="Address" value={formattedAddress?.split("\n").map((line) => <span key={line} className="block">{line}</span>)} status={isCmsRecord ? "CMS address fields with geocoded coordinates" : syntheticValue} />
      ...
      <section className="mt-5" aria-labelledby="contact-access-heading">
        ...
        <dl className="mt-3 rounded-lg border border-slate-200 bg-white px-4">
          <DetailRow label="Phone" value={phoneDisplay && phoneLink ? <a className="text-terrain-700 underline" href={phoneLink}>{phoneDisplay}</a> : phoneDisplay} status={phoneDisplay ? "CMS telephone_number" : undefined} />
          <DetailRow label="Website" value={websiteLink ? <a className="text-terrain-700 underline" href={websiteLink} target="_blank" rel="noopener noreferrer">Visit facility website</a> : undefined} status={websiteLink ? "Source-backed facility website" : undefined} />
      ...
      <section className="mt-5" aria-labelledby="cms-services-heading">
        <h2 id="cms-services-heading" className="text-sm font-semibold uppercase tracking-wide text-slate-500">Services reported by CMS</h2>
        <dl className="mt-3 rounded-lg border border-slate-200 bg-white px-4">
          <DetailRow label="Emergency services" value={emergencyValue} status="CMS emergency-services designation is not live operational status." />
        </dl>
      </section>
      ...
      <section className="mt-5" aria-labelledby="source-quality-heading">
        ...
        <dl className="mt-3 rounded-lg border border-slate-200 bg-white px-4">
          ...
          <DetailRow label="Source dataset" value={facility.sourceName ?? (sourceRecords.map((s) => s?.dataset).join("; ") || undefined)} />
          <DetailRow label="Source dataset ID" value={facility.sourceDatasetId} />
          ...
          <DetailRow label="Retrieved date" value={facility.retrievedAt ? formatDate(facility.retrievedAt) : (retrievalDates.length ? retrievalDates.map(formatDate).join("; ") : undefined)} />
          <DetailRow label="Validation status" value={facility.validationStatus} />
          <DetailRow label="Geocoding confidence" value={facility.geocodingConfidence} />
          <DetailRow label="Source link" value={facility.sourceUrl ?? (sourceLinkCount ? `${sourceLinkCount} source link(s) listed below` : undefined)} />
        </dl>
      ...
      <section className="mt-5 rounded-lg bg-slate-50 p-4 text-sm text-slate-600" aria-labelledby="known-limitations-heading">
        ...
        <ul className="list-disc space-y-1 pl-4">
          ...
          <li>CMS emergency-services designation is not live operational status.</li>
