...
test('slider text and quick controls update same callback',()=>{assert.match(sidebar,/type="range"/);assert.match(sidebar,/id="radius-number"/);assert.match(sidebar,/onRadiusChange\(parsed\)/);assert.match(sidebar,/onRadiusChange\(miles\)/);assert.match(sidebar,/onRadiusChange\(Number\(event\.target\.value\)\)/);});
