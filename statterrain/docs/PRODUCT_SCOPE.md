...
## v0.3.5.2 search and ACS scope boundary

The permanent planning-location search contract covers full street address, city plus state, ZIP/ZIP+4, latitude/longitude, and map click. The same PlanningLocation transaction drives map recentering, the planning marker, CMS partition selection, radius filtering, summaries, and Evidence Brief metadata. City/state and ZIP searches use official Census-backed representative geography points and disclose their limitations. Coordinate searches are local; invalid coordinates preserve the prior planning location. ACS population ingestion remains out of scope until v0.3.6.
