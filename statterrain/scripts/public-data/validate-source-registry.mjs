...
const allowed = { sourceType: new Set(["official-public-data"]), status: new Set(["planned"]), dataMode: new Set(["not-yet-ingested"]), refreshMethod: new Set(["manual-planned"]), automationStatus: new Set(["not-started"]), lastValidationStatus: new Set(["not-run"]) };
const errors = [];
let registry;
...
for (const [index, source] of (registry.sources ?? []).entries()) {
  ...
  if (source.usedInCurrentApp !== false) errors.push(`${source.id}: usedInCurrentApp must be false in v0.2.0.`);
